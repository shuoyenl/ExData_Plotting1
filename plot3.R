...
partialData <- read.table(sourcefile, sep=";", header=TRUE, na.strings=c("?"), nrows = 100)
classes <- sapply(partialData, class)
allData <- read.table(sourcefile, sep=";", header=TRUE, na.strings=c("?"), colClasses=classes, skip = 0)
